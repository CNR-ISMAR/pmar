...
class LagrangianDispersion(object): 
    ...
    def polygon_grid(self, r_bounds=None, force=False):
        ...
        self._x_grid = np.array(cols)
        self._y_grid = np.array(rows)
        return intersect
    ...
    def particle_raster(self, res=0.04, crs='4326', tinterp=None, r_bounds=None, use_path='even', use_label='even', decay_rate=None, aggregate='mean', depth_layer='full_depth', z_bounds=[1,-10], particle_status='all', traj_dens=False, save_r=True):
        ...
        for idx, path in enumerate(particle_path):
            ...
            if use_path != 'even':
                ...
                use = rxr.open_rasterio(use_path).rio.reproject('epsg:4326', nodata=0).sortby('x').sortby('y').isel(band=0).drop('band').sel(x=slice(poly_bounds[0], poly_bounds[2]), y=slice(poly_bounds[1], poly_bounds[3])).fillna(0)
                use_weight = use.sel(x=ds.isel(time=0).lon, y=ds.isel(time=0).lat,  method='nearest', tolerance=res*2) ### TODO capire bene tolerance
