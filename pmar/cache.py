...
class PMARCache(object):
    ...
    def raster_cache(self, context, pressure, chemical_compound, seeding_shapefile, seeding_radius, res, pnum, ptot, duration, start_time, seedings, seeding_id, tshift, use, use_label, emission, decay_coef, study_area):
        cache_key = {'context': context, 'pressure': pressure, 'chemical_compound': chemical_compound, 'seeding_shapefile': seeding_shapefile, 'seeding_radius': seeding_radius, 'res': res, 'pnum': pnum, 'ptot': ptot, 'duration': duration, 'start_time': start_time, 'seedings': seedings, 'seeding_id': seeding_id, 'tshift': tshift, 'use': use, 'use_label': use_label, 'emission': emission, 'decay_coef': decay_coef, 'study_area': study_area}
