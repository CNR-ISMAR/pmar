...
class PMARCache(object):
    ...
    def raster_cache(self, context, pressure, chemical_compound, seeding_shapefile, seeding_radius, res, poly_path, pnum, ptot, duration, start_time, seedings, tshift, use_path, use_label, emission, decay_coef, r_bounds):
        cache_key = {'context': context, 'pressure': pressure, 'chemical_compound': chemical_compound, 'seeding_shapefile': seeding_shapefile, 'seeding_radius': seeding_radius, 'res': res, 'poly_path': poly_path, 'pnum': pnum, 'ptot': ptot, 'duration': duration, 'start_time': start_time, 'seedings': seedings, 'tshift': tshift, 'use_path': use_path, 'use_label': use_label, 'emission': emission, 'decay_coef': decay_coef, 'r_bounds': r_bounds}
